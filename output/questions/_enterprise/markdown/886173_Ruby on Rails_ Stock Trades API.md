...
- **Tags:** Ruby on Rails, Rails, Ruby, Medium, REST API
- **Skills:** RoR (Intermediate)

## Summary

This back-end development question evaluates REST API implementation, JSON data handling, and trade management concepts, ideal for mid-level roles. The task involves creating a REST API to manage stock trades with specific endpoints and response requirements.
...
- optionally accepts query parameters trade_type and user_id, for example `/trades/?trade_type=buy&user_id=122`. All these parameters are optional. In case they are present, only objects matching the parameters must be returned.
