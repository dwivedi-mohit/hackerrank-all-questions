# Django REST Framework: Movie Ratings API
...
- **Tags:** Medium, Serializers, Views, Django 3.0.6, Python 3.7, Django REST Framework
- **Skills:** Django (Intermediate)

## Summary

This back-end development question evaluates REST API implementation, data retrieval, and data manipulation concepts, ideal for mid-level roles. The problem requires creating a REST API to manage movies and ratings, ensuring correct handling of requests and responses as specified.
...
Here is an example of a movie JSON object:
...
Here is an example of a rating JSON object:
...
- if the given movie doesn't exist, the response code is 404
...
Your task is to complete the given project so that it passes all the test cases when running the provided unit tests. The project by default supports the use of the SQLite3 database. Implement the POST request to `/movies/` first because testing the other methods requires it to work correctly.
...
In this challenge, your task is to implement a simple REST API to rate movies.
