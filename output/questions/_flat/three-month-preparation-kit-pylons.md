...
| Field | Value |
|---|---|
| **Slug** | `three-month-preparation-kit-pylons` |
| **Domain** |  |
| **Difficulty** | Medium |
| **Score** | 100 |
| **URL** | https://www.hackerrank.com/challenges/three-month-preparation-kit-pylons |

---

## Preview

Find the minimum number of towers Goodland's government must switch on to bring power to its citizens.
...
Each city is $1$ unit distance from its neighbors, and we'll use $0$ based indexing.  We see there are $3$ cities suitable for power plants, cities $1, 2$ and $3$.  If we build a power plant at $arr[2]$, it can serve $arr[0]$ through $arr[4]$ because those endpoints are at a distance of $2$ and $2 \lt k$.  To serve $arr[6]$, we would need to be able to build a plant in city $4, 5$ or $6$.  Since none of those is suitable, we must return -1.  It cannot be done using the current distribution constraint.
...
* $1 \leq k \leq n\leq 1000$ for $\text{40%}$ of the maximum score.
