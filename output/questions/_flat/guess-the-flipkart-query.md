...
| Field | Value |
|---|---|
| **Slug** | `guess-the-flipkart-query` |
| **Domain** | ai |
| **Difficulty** | Hard |
| **Score** | 30 |
| **URL** | https://www.hackerrank.com/challenges/guess-the-flipkart-query |

---

## Preview

Given the name of a product listed on Flipkart, identify which of ten given queries, led to it.

## Problem Statement

[Flipkart](http://www.flipkart.com/) is a popular Indian e-commerce portal.
One of the most common actions performed by users of the portal, is to use the search box and query for a brand, product or product-line. The search box then returns the best matching products which it can find - along with their prices, details, descriptions, etc.
...
The product names will not exceed **200 characters** in length.
Sometimes, when the product name is long and descriptive, after the first 55 characters, there are likely to be a series of dots, such as the examples below. Please handle them appropriately (strip them off, or ignore them).  
...
The i<sup>th</sup> line should contain the query (your best guess) which returned the i<sup>th</sup> product name in the input file. 
The query should strictly be from one of the twenty queries specified above, **as is**. Please do not add any leading or trailing spaces or any extra punctuation. Also ensure that the case remains the same.
...
The training file can also be opened from your program, during execution.
It can be opened using the name "training.txt" and is available in the same directory where the program is being run.
...
A small training file with a few examples of products returned for the various search queries is available. 
Please note, that this is only a **small** training file, and it is expected that a mix of simple and creative ideas from machine learning, string matching and information retrieval will be used in the submitted solution.
...
Your score for a test case = C/N * M where:
    ...
    C = search queries correctly identified
    N = Total number of product names in the test case
