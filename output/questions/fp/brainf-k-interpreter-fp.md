...
| Field | Value |
|---|---|
| **Slug** | `brainf-k-interpreter-fp` |
| **Domain** | fp |
| **Difficulty** | Advanced |
| **Score** | 100 |
| **URL** | https://www.hackerrank.com/challenges/brainf-k-interpreter-fp |

---

## Preview

Write a brainf__k interpreter.

## Problem Statement

BrainF\_\_k is an esoteric programming languages. It is designed to provide a tongue-twister to programmers, where even adding two numbers can be more difficult than writing a complex algorithm in imperative languages.

For this problem, a BrainF\_\_k program is allocated a continuous memory of infinite bytes, where memory locations are indexed from 0 onwards.
...
Given a valid BrainF\_\_k program and an input string, you have to print the result of the program when executed. All those characters of the program which does not represent a valid command can be considered as comment and should be ignored.

You have to print the output for first 10<sup>5</sup> operations. If program executes more than 10<sup>5</sup> operations then you have stop execution and print "`PROCESS TIME OUT. KILLED!!!`" (without quotes) in the next line.
...
First line will contain two space separated integers, `n m`, which represent number of characters in input to BrainF\_\_k program and number of lines in the program, respectively. Next line contains `n+1` characters which represents the input for the BrainF\_\_k program. This line ends with character '`$`' which represent the end of input. Please ignore this in input. Then follows `m` lines which is the BrainF\_\_k program.

**Output**

You have to print the output of program as mentioned in _Objective_. For programs with more than 100000 operations, print the output till then followed by "`PROCESS TIME OUT. KILLED!!!`" in the next line.
